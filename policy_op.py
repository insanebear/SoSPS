    ...
    for role in value_map[5]:   # role = 1, 2, 3
        ...
        for action in role_action_map[role]:
            ...
            for combination in combinations:
                ...
                policy[5] = role
                policy[16] = -1
                policy[17] = True       # TODO Change "Enforce factor should be handled."
...
def mut_policy(policy_set, m_portion):
    ...
    for i in range(0, len(policy_set)*m_portion):
...
#     for idx in candid_cond_list:
#         policy[idx] = select_value_by_idx(idx)
#
#     # make rest of indices to invalid(-1)
#     for pol_idx in range(0, len(policy)):
#         if policy[pol_idx] is None:
...
#
#     if mut_type_prob < 0.2:         # mutation by addition
#         policy = gen_policy()
#         while not check_policy(policy):
#             policy = gen_policy()
#             if check_policy(policy):
#                 break
#         policy_set.append(policy)
#     elif mut_type_prob < 0.4:       # mutation by deletion
#         if len(policy_set) > 2:
#             del_idx = random.randint(0, len(policy_set)-1)
#             del policy_set[del_idx]
#     elif mut_type_prob < 1.0:       # mutation by modification
...
make_policy_json("newPolicy.json", policies)
print(len(policies))
